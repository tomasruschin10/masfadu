...
export class ResourceService {
    ...
    async create(request: any, file){
      ...
      const resource = await this.resourceRepository.create(request)
      if (!resource) throw new HttpException('incorrect data',HttpStatus.BAD_REQUEST)   
   ...
   async update(id:number, request: any, file){
      ...
      const resource = await this.resourceRepository.update(id, request)

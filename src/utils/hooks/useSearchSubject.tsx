...
function useSearchSubject() {
    ...
    useEffect(() => {
        setLoading(true)
        getServices('subject/all').then(({data}: any) => {
            setAllSubjects(data)
        }).catch(error => {
            if(__DEV__){
            ...
            const delayDebounceFn = setTimeout(() => {
                const result = allSubjects.filter (
                        subj => subj.name.toLowerCase()
                                        .normalize("NFD")
                                        .replace(/[\u0300-\u036f]/g, "")
                                        .includes(search.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")) // elimina caracteres extraños
                    )
    ...
    return { search, setSearch, filteredSubjects, setFilteredSubjects, allSubjects, loading }
}

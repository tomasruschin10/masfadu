...
function ChangePassword({ route, navigation }) {
  ...
  const sendForm = () => {
    if (form.newpassword === repeatPassword) {
      putServices(`auth/update/${userdata.id}`, form)
        .then(({ data }: any) => {
          store.dispatch(updateUserdata(data));
          dispatch(
            updateMessage({
              body: "Contraseña Guardada con Exito!",
              open: true,
              type: "success",
            })
          );
        ...
        .catch((err) =>
          dispatch(
            updateMessage({
              body: "Asegurese de haber escrito correctamente su contraseña",
              open: true,
              type: "danger",
            })
          )
        ...
        <Box mt="6" mb={24} marginX="auto">
          ...
          >
            <Text color="brand.primary" fontWeight={"500"}>
              Uy, no me acuerdo la contraseña
            </Text>
          </TouchableOpacity>
        </Box>

...
function On2Screen({ route, navigation }) {
	...
	const applyCareer = () => {
		putServices(`auth/update/${userdata.uid}`,{career_id: carrier}).then((res: any) => {
			setCareer(res.data)
		}).catch(error => {
			if(__DEV__){
				console.log("🚀 ~ file: On2Screen.tsx ~ line 21 ~ getServices ~ error", error)
		...
		<Box backgroundColor={"background"} h='100%'>
			...
			<ScrollView>
				...
				<Box px={5} alignContent={"center"} mb={10} mt={2}>
					...
					<Box my={10} flexDir={'row'} justifyContent={'space-between'} flexWrap={'wrap'}>
						{ career.length > 0 ?
							career.map(item => {
								return (
									<ButtonBordered
			...
			<Button position={'absolute'} opacity={carrier < 1 ? 0.5 : 1} disabled={carrier < 1} bottom={5} onPress={()=> applyCareer() } mb={3} w="90%" alignSelf={'center'}>
				Siguiente
			</Button>
		</Box>
	);

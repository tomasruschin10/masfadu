...
function SeeSubjectThread_Top({title, text, time, hours, method, navigation, setToggle, toggle, fadeAnim, color, firstLetter}) {
  return (
    <HStack rounded="3xl" bg='white' p={4} shadow={2} my={1} mx={5} space={3}>
      <Box>
        ...
        <Avatar  h={'100%'}  rounded={5} _text={{fontSize: (30/firstLetter.length)+(firstLetter.length*1.5)}} fontFamily='SourceSansPro_400Regular' fontSize={40} bg={color} mr="1" >{firstLetter}</Avatar>
      ...
      <Box flex={1}>
        <Text fontSize={13} fontWeight={'500'}>{title}</Text>
        ...
        <Text fontFamily={'SourceSansPro_400Regular'} numberOfLines={2} fontSize={12}>{time}</Text>
      </Box>
    </HStack>
  )
}

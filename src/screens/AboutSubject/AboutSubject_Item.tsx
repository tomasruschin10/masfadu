...
function AboutSubject_Item({ subjCategory, nav, updater, setUpdater }) {
    ...
    const [infoUserSubj, setInfoUserSubj] = useState({
        user_id: 0, subject_id: 0, score: 0, finish: 0, extra_score: [], USERSUBJECT: 0
    })
    ...
    return (
        <>
            ...
            <Box bg={'white'} borderColor={'#D4D4D4'} borderWidth={1} rounded={'xl'} mt={1} mb={4} mx={5}>
                <HStack p={4} justifyContent="space-between">
                    <Box flex={1}>
                        <Text color={'gray.500'} fontWeight={'600'} fontSize={13}>Materias aprobadas</Text>
                        <Text color={'gray.400'} fontSize={31} mt={2}>{`${subjCategory.on}/${subjCategory.total}`}</Text>
                        <Box bg={'#EBEEF2'} rounded={'full'} height={2}>
                            <LinearGradient start={{x: -1, y: 0}} end={{x: 1, y: 0}} colors={['#CCCED1', '#3A71E1','#1ABC9C' ]} style={{height: '100%',width:`${subjCategory.total !== 0 ? 100 / subjCategory.total * subjCategory.on : 100 / 1 * subjCategory.on}%`, borderRadius:10
                            }} />
                        </Box>
                    </Box>
                    <Box ml={4}>
                        <Text color={'gray.500'} fontWeight={'600'} fontSize={13}>Promedio</Text>
                        <Box bg={'#F7F7F7'} rounded={'xl'} mt={2}>
                            <Text color={'gray.400'} textAlign={'center'} fontSize={28} py={1}>
            ...
            {subjCategory.data.map((item, index) => (
                <SimpleAccordion
                    ...
                    titleStyle={{ color: "#3A71E1", fontWeight: "600", fontFamily: "Poppins_400Regular", fontSize: 17.61 }}
                    bannerStyle={[
                        {

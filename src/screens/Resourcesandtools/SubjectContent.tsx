...
function SubjectContent({ route, navigation }) {
  ...
  const byWords = () => {
    setLoading(true);
    getServices(`resource-category/all?search=${searchText.trim()}`)
      .then(({ data }: any) => {
        setResourceCategory(data);
        setSearchText("");
      })
  ...
  const shareFile = async (image) => {
    ...
    const fileUri =
      FileSystem.documentDirectory + encodeURI(image.name).slice(23);
  ...
  return (
    <Container>
      {/* - ${viewName} */}
      <Layout route={route} navigation={navigation} title={`${name} `}>

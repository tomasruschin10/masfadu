...
export const Item = ({id, name, image, description, idPress, setIdPress }) => (
  <Box mt={1} w={'50%'} mb="2">
    <TouchableHighlight underlayColor="" onPress={() => id === idPress ? setIdPress(90000) : setIdPress(id)}>
      <Box shadow={'5'} style={styles.shadow} bgColor={'white'} rounded="md" mx="1.5">
        <Image source={{uri: image.url}} alt="Career_Image" mb={2} h={150} borderTopRadius={'md'} />
        <Text mx={2} textAlign={'center'} bold={true}>{name}</Text>
        <Text mx={2} textAlign={'center'} fontSize={12}>{id === idPress && description}</Text>
      </Box>
    </TouchableHighlight>
  </Box>
)
...
function LostObjects({route, navigation}) {
  ...
  useEffect(() => {
      setLoading(true)
      getServices('lost-object/all').then((res: any) => {
			setObjects(res.data)
		}).catch(error => {
			if(__DEV__){
  ...
  return (
    <>
      ...
      <Box mt="7" position={'absolute'} bottom={32} alignSelf={'center'}>
        <Button rounded={'xl'} fontWeight="bold" onPress={() => Linking.openURL('mailto:hola@masfadu.com')} mx="5" py={3}>Reconocés o encontraste uno?</Button>
      </Box>
    </>
  ) 
}
